...
{
    lpc_parcel_t parcel = mem_alloc(PORT_MSG_SIZE);
    if (parcel) {
...
int lpc_do_echo(uint32_t port, lpc_handler_t func)
{
    ...
    msg_recv = mem_alloc(sizeof(port_msg_t));
    if (msg_recv == NULL) {
        errprint( "lpc: mem_alloc for recv msg failed!\n");
        return -1;
    }
    msg_reply = mem_alloc(sizeof(port_msg_t));
    if (msg_reply == NULL) {
        errprint( "lpc: mem_alloc for reply msg failed!\n");
        mem_free(msg_recv);
        return -1;
    }
#else
    ...
    while (1) {
        ...
        if (sys_port_comm_receive(port, msg_recv) < 0)
            continue;
        /* process msg */
        bool result = false;
        ...
        if (func)
            result = func(
                        ...
                        reply_parcel);
        if (!result) {
            errprint( "lpc: port %d do serv func failed!\n", port);
        }
        ...
        if (sys_port_comm_reply(port, msg_reply) < 0) {
            errprint( "lpc: reply port %d failed!\n", port);
        }
    }
}
...
{
    if (sys_port_comm_bind(port, 0) < 0) {
        errprint( "lpc: bind port %d failed!\n", port);
        return -1;
    }
...
{
    if (sys_port_comm_bind(port, PORT_BIND_GROUP) < 0) {
        errprint( "lpc: bind port group %d failed!\n", port);
        return -1;
    }
    return lpc_do_echo(port, func);
}
...
int lpc_call(uint32_t port, uint32_t code, lpc_parcel_t data, lpc_parcel_t reply)
{
    ...
    /* 分配请求消息 */
    if (!msg) {
        msg = mem_alloc(sizeof(port_msg_t));
        if (!msg)
            return -1;
    }
#else
...
#endif
    /* 绑定一个随机端口 */
    if (sys_port_comm_bind(-1, PORT_BIND_ONCE) < 0) {
        #if LPC_MSG_USE_MALLOC == 1
        mem_free(msg);
        #endif
        return -1;
    ...
    if (sys_port_comm_request(port, msg) < 0) {
        #if LPC_MSG_USE_MALLOC == 1
        mem_free(msg);
        #endif
        return -1;
