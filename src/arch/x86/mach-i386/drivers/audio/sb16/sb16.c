...
bool sb16_initialize(device_extension_t *extension)
{
    ...
    if (data != 0xaa)
    {
        keprint(PRINT_NOTICE "sb16: sb not ready");
        return false;
    }
    ...
    keprint(PRINT_INFO "sb16: found version %d.%d\n", extension->major_version, vmin);
    vmin = 0;
    return true;
}
void sb16_request(device_extension_t *extension)
{
    ...
    #ifdef DEBUG_SB16
    keprint(PRINT_DEBUG "sb16: [DMA] [%x] sample_count: %d\n", dma_region->v, length);
    #endif
}
...
static int sb16_handler(irqno_t irq, void *data)
{
    ...
    {
        #ifdef DEBUG_SB16
        keprint(PRINT_DEBUG "sb16: [NEW READ]\n");
        #endif
...
static ssize_t __sb16_write(device_extension_t *extension, const u8 *data, ssize_t length)
{
    ...
    #ifdef DEBUG_SB16
    keprint(PRINT_DEBUG "sb16: [WRITE] [%x] %d\n", dma_region->v, length);
    #endif
...
static iostatus_t sb16_write(device_object_t *device, io_request_t *ioreq)
{
    ...
    {
        status = IO_FAILED;
        keprint(PRINT_ERR "sb16: write fialed!\n");
    }
...
static iostatus_t sb16_enter(driver_object_t *driver)
{
    ...
    {
        keprint(PRINT_ERR "sb16_enter: create device failed!\n");
        return status;
    }
    ...
    {
        #ifdef DEBUG_SB16
        keprint(PRINT_INFO "[i:%d][count:%d]\n", i, DMA_COUNT);
        #endif
        ...
        {
            keprint(PRINT_ERR "sb16_enter: alloc dma buffer failed!\n");
            io_delete_device(devobj);
            status = IO_FAILED;
            return status;
        }
        #ifdef DEBUG_SB16
        keprint(PRINT_INFO "sb16: alloc dma buffer vir addr %x phy addr %x\n", extension->dma_region[i].v, extension->dma_region[i].p.address);
        #endif
    }
...
#ifdef DEBUG_SB16
    keprint(PRINT_DEBUG "sb16_driver_func: driver name=%s\n",
           driver->name.text);
#endif
    return status;
}
